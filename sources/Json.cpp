...
bool Json::is_array() const { return _is_array; }

bool Json::is_object() const { return _is_object; }
...
void Json::create_vector(const string& s) {
  ...
  while (i < s.find_last_of("]")) {
    ...
    if (s[i] == '{') {
      ...
      if (n == s.length()) throw std::logic_error("string is not valid");
      s1 = s.substr(i, n - i + 1);
      Json obj(s1);
...
void Json::create_map(const string& s) {
  ...
  while (i < s.find_last_of("}")) {
    ...
    key = read_key(i, s);
    i = miss_spaces(i, s);
    if (s[i] != ':') throw std::logic_error("String is not valid");
    i++;
    i = miss_spaces(i, s);
    if (s[i] == '{') {
      string s1;
